...
class ClothesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'all websites return clothes' do
    ...
    categories.each do |category|
      websites = WebsiteUrlHtml.where(category: category).map(&:website)
      websites.each do |website|
        puts "website: #{website[:name]}, gender: #{
        ...
        post '/clothes/get_clothes',
             params: {
               ...
               page: '1',
               websites: JSON.generate([website[:id]])
             }
        assert_not_empty response.body,
                         "#{website[:name]} didn't return any clothes"
      end
    end
  end
end
